...
class GaussianModel:
    ...
    def get_scaling_base(self):
        # Read-only NaN/overflow protection only (does NOT modify _scaling_base)
        # Widened to (-20, 20) to match LocoGS behavior (no clamp in LocoGS)
        clamped_scaling = torch.clamp(self._scaling_base, -20.0, 20.0)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # Initialize SH mask same as LocoGS: sigmoid(1.0)≈0.731, balanced start for optimization
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, iteration=None):
        ...
        # Align with LocoGS: log(exp(scaling_base) / (0.8*N)) = scaling_base - log(0.8*N)
    ...
    def maybe_densify_and_prune(
        ...
        extent: float,
        max_screen_size,
        mask_prune_threshold: float = 0.01,
    ):
        """
        Densification schedule wrapper.
        
        Design:
        - Densification within schedule (iteration < densify_until_iter).
        ...
        elif iteration % prune_interval == 0:
